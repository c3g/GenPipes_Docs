...
* The command syntax to run GenPipes deployed in the `Digital Research Alliance of Canada <https://alliancecan.ca/en>`_, formerly Compute Canada (CCDB) servers, such as Beluga, has **changed**. Pipelines are now invoked with `genpipes <pipeline> -h` instead of `<pipeline>.py -h`. See example below:
